...
class Booking(models.Model):
    ...
    def __str__(self):
        return f"{self.item} - {self.item.owner} - {self.start_data} - {self.end_data}"

    def save(self, *args, **kwargs):
        if self.end_data < self.start_data:
            raise ValidationError("Start date must be before end date")
        qs = Booking.objects.filter(item=self.item, start_data__lte=self.end_data,
                                    end_data__gte=self.start_data).exclude(status=StatusBooking.REJECTED).exclude(pk=self.pk)
        if qs.exists():
